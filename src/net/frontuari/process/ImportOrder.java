...
public class ImportOrder extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		{
			sql = new StringBuilder ("DELETE I_FTUOrder ")
		...
		sql = new StringBuilder ("UPDATE I_FTUOrder ")
		...
		sql = new StringBuilder ("UPDATE I_FTUOrder o ")	//	SO Document Type Name
			  .append("SET C_DocType_ID=(SELECT C_DocType_ID FROM C_DocType d WHERE d.Name=o.DocTypeName")
		...
		sql = new StringBuilder ("UPDATE I_FTUOrder ")	//	Error Invalid Doc Type Name
		...
		//	DocType Default
		sql = new StringBuilder ("UPDATE I_FTUOrder o ")	//	Default PO
		...
		sql = new StringBuilder ("UPDATE I_FTUOrder o ")	//	Default SO
			  .append("SET C_DocType_ID=(SELECT MAX(C_DocType_ID) FROM C_DocType d WHERE d.IsDefault='Y'")
			  .append(" AND d.DocBaseType='SOO' AND o.AD_Client_ID=d.AD_Client_ID) ")
		...
		sql = new StringBuilder ("UPDATE I_FTUOrder ")	// No DocType
		...
		//	Set IsSOTrx
		sql = new StringBuilder ("UPDATE I_FTUOrder o SET IsSOTrx='Y' ")
		...
		sql = new StringBuilder ("UPDATE I_FTUOrder o SET IsSOTrx='N' ")
		...
		//	Set Country
		/**
		sql = new StringBuffer ("UPDATE I_FTUOrder o "
		...
		**/
		...
		sql = new StringBuilder ("UPDATE I_FTUOrder o ")
		  ...
		  .append("WHERE C_BankAccount_ID IS NULL AND AccountNo IS NOT NULL")
		  .append(" AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE)) log.fine("Set Tax=" + no);
		sql = new StringBuilder ("UPDATE I_FTUOrder ")
		  .append("SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid BankAccount, ' ")
		  .append("WHERE C_BankAccount_ID IS NULL AND AccountNo IS NOT NULL")
		  .append(" AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no != 0)
		log.warning ("Invalid BankAccount=" + no);
		*/
		...
		sql = new StringBuilder ("SELECT * FROM I_FTUOrder ")
...
//						New Location
		...
		sql = new StringBuilder ("SELECT * FROM I_FTUOrder ")
			...
			.append(" ORDER BY C_BPartner_ID, BillTo_ID, C_BPartner_Location_ID, I_FTUOrder_ID");
		try
		{
			pstmt = DB.prepareStatement (sql.toString(), get_TrxName());
			rs = pstmt.executeQuery ();
