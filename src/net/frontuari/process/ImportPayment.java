...
public class ImportPayment extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		{
			sql = new StringBuilder ("DELETE I_FTUPayment ")
		...
		sql = new StringBuilder ("UPDATE I_FTUPayment ")
		...
		sql = new StringBuilder ("UPDATE I_FTUPayment o ")
		...
		//		Order
		sql = new StringBuilder ("UPDATE I_FTUPayment i ")
			  .append("SET C_Order_ID=(SELECT MAX(C_Order_ID) FROM C_Order ii")
			  .append(" WHERE i.c_order_documentno=ii.DocumentNo AND i.AD_Client_ID=ii.AD_Client_ID) ")
			  .append("WHERE C_Order_ID IS NULL AND c_order_documentno IS NOT NULL")
			  .append(" AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no != 0)
		...
		if (no != 0)
			if (log.isLoggable(Level.FINE)) log.fine("Set Invoice from DocumentNo=" + no);
		// Invoice from c_order_id
		sql = new StringBuilder ("UPDATE I_FTUPayment i ")
			  .append("SET C_Invoice_ID=(SELECT MAX(C_Invoice_ID) FROM C_Invoice ii")
			  .append(" WHERE i.C_Order_ID=ii.C_Order_ID AND i.AD_Client_ID=ii.AD_Client_ID) ")
		...
		sql = new StringBuilder ("UPDATE I_FTUPayment i ")
		  .append("SET C_BPartner_ID=(SELECT MAX(C_BPartner_ID) FROM C_Order ii")
		  .append(" WHERE i.C_Order_ID=ii.C_Order_ID AND i.AD_Client_ID=ii.AD_Client_ID) ")
		...
		sql = new StringBuilder("UPDATE I_FTUPayment ")
			...
			.append("WHERE I_FTUPayment_ID IN ")
				.append("(SELECT I_FTUPayment_ID ")
				.append("FROM I_FTUPayment i")
		...
		try
		{
			...
			while (rs.next())
			{ 
				X_I_FTUPayment imp = new X_I_FTUPayment(m_ctx, rs, get_TrxName());
			...
			//	payment.setDescription(imp.getDescription());
				...
				int invoiceId= imp.getC_Invoice_ID();
				if(invoiceId>0){
					payment.setC_Invoice_ID(invoiceId);
				}else{
					payment.setC_Order_ID(imp.getC_Order_ID());
